...
> [!WARNING]
> **Render free tier limits:** the web service spins down after 15 minutes of inactivity, and uploaded photos are lost on restart (ephemeral disk). The free PostgreSQL instance expires after 90 days. A paid plan is recommended for a live wedding website.
...
```bash
cp .env.example .env
# Open .env — set SECRET_KEY, DB_USER, and DB_PASS to random values before first run.
# Generate values with: python3 -c "import secrets; print(secrets.token_urlsafe(24))"

docker compose -f docker/docker-compose.yml --env-file .env up -d
```

> **Security:** `.env.example` ships with placeholder credentials — always replace `SECRET_KEY`, `DB_USER`, and `DB_PASS` with strong unique values before starting the containers.
...
```bash
# Fill in DB_HOST, DB_USER, DB_PASS, DB_NAME in .env (or set DATABASE_URL directly)
